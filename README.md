...
Description: The name of the Azure CDN profile resource.
...
Description: The full Azure Resource ID of the resource group where the CDN profile will be deployed.
...
Description: The pricing tier (SKU) of the CDN profile. This determines which features are available and the pricing structure.
...
Description: A map of Azure Front Door (AFD) endpoints to create within the CDN profile. Each endpoint can have multiple routes configured for content delivery.
...
Description: A map of custom domains to configure for the CDN profile. Custom domains allow you to use your own domain names with the CDN.
...
Description: The Azure region where the CDN profile resource metadata will be stored.
...
Description: Controls the Resource Lock configuration for the CDN profile. Resource locks prevent accidental deletion or modification of critical resources.
...
Description: Defines log scrubbing rules that remove or mask sensitive information from Azure Front Door profile logs before they are stored or analyzed.
...
- `scrubbing_rules` = (Optional) A list of scrubbing rules to apply to the logs. Defaults to an empty list.
  ...
  - `selector`                = (Optional) The specific selector to match (e.g., header name, query parameter name). If not specified, all instances of the match\_variable will be scrubbed.
...
Description: A map of origin groups to create in the CDN profile. Origin groups define a collection of backend origins and the load balancing/health probe settings for them.
...
Description: The timeout in seconds for forwarding requests to and receiving responses from the origin servers.
...
Description: Controls whether private DNS zone groups should be managed by this module for private endpoints.
...
Description: A map of rule sets to create in the CDN profile. Rule sets contain routing rules that modify request/response behavior based on conditions.
...
Description: A map of secrets (certificates) to create in the CDN profile. Secrets are used for TLS/SSL certificates on custom domains.
...
Description: A map of security policies to create in the CDN profile. Security policies associate Web Application Firewall (WAF) rules with domains and routes.
...
Description: A map of tags to assign to the CDN profile resource.
...
Description: A map of target groups to create in the CDN profile. Target groups define collections of backend endpoints for tunnel policies.
...
Description: A map of tunnel policies to create in the CDN profile. Tunnel policies enable secure tunneling functionality through Azure Front Door.
